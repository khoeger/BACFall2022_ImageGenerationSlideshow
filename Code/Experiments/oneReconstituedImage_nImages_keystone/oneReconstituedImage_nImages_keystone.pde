...
void setup() {
  ...
  surface = ks.createCornerPinSurface(1000, 1000, 30);
  offscreen = createGraphics( 876, 1068, P3D);
  //ks.background(baseColor);

  //size(876,1068);
  //size(1000, displayHeight);

    offscreen.beginDraw();
    offscreen.background(baseColor);
    offscreen.endDraw();
    ks.load();
  randomSeed(seed);
  ...
  for (int currentImageIndex = 0; currentImageIndex < imageStringArray.length; currentImageIndex ++ ) {
    ...
    //}
    img.resize( offscreen.width  - 2 * borderSpace, 
      offscreen.height - 2 * borderSpace );
  ...
  myFont = createFont(fontName, floor( 0.45*min(offscreen.width, offscreen.height)));
  textFont(myFont);
